...
# Physical GPUs used by the standalone fixed-seed evaluator (one shard each).
...
ROLLOUT_INFERENCE_MICRO_BATCH_SIZE=${ROLLOUT_INFERENCE_MICRO_BATCH_SIZE:-8}
ACTOR_LOGPROB_RECOMPUTE_MICRO_BATCH_SIZE=${ACTOR_LOGPROB_RECOMPUTE_MICRO_BATCH_SIZE:-4}
REFERENCE_FORWARD_MICRO_BATCH_SIZE=${REFERENCE_FORWARD_MICRO_BATCH_SIZE:-4}
POST_UPDATE_LOGPROB_AUDIT_MICRO_BATCH_SIZE=${POST_UPDATE_LOGPROB_AUDIT_MICRO_BATCH_SIZE:-4}
...
REFERENCE_TARGET_KL=${REFERENCE_TARGET_KL:-0.05}
MAX_OPTIMIZER_STEPS_PER_UPDATE=${MAX_OPTIMIZER_STEPS_PER_UPDATE:-15}
...
if [[ ! -f "${BASE_EVAL_ROOT}/summary.json" ]]; then
    BASE_CHECKPOINT="${EVAL_MODEL_BASE_CHECKPOINT}" NUM_SHARDS="${EVAL_NUM_SHARDS}" GPU_IDS="${EVAL_GPU_IDS}" bash \
